...
%
% [SOH, FEC, Aircraft] = CyclAging(Aircraft, ChemType, CumulFECs, ChargingTime, ChrgRate)
...
ValiColumn = find(any(Power.Capacity ~= 0, 1));
...
SOC   = Power.SOC;
SOCcol = SOC(:, ValiColumn);
DOD = max(SOCcol) - min(SOCcol);

% C-rates during discharging %
DCr = Power.C_rate;
DisCCrate = mean(DCr(DCr ~= 0));

% C-rates during charging %
Aircraft = BatteryPkg.GroundCharge(Aircraft, ChargingTime, ChrgRate);
Power  = Aircraft.Mission.History.SI.Power;
ChAC = Power.ChargedAC;
CCr  = ChAC.C_rate;
CCrate = mean(CCr(CCr ~= 0));

% mean/median SOC %
active_mSOC = SOCcol([true; diff(SOCcol) ~= 0]); % Remove consecutive repeated SOC values
...
Cap      = Power.Capacity;
CapCol   = Cap(:, ValiColumn);
CapColNZ = CapCol(CapCol ~= 0);
dDis     = max(CapCol) - min(CapColNZ);

ChCap      = ChAC.Capacity(:);
ChCapNZ    = ChCap(ChCap ~= 0);
dCh        = max(ChCap) - min(ChCapNZ);

ParCells = Aircraft.Specs.Power.Battery.ParCells;
FEC = (dDis + dCh) / (2 * QMax * ParCells) + CumulFECs; % flight FEC for this single mission (charge + discharge)

%% THE FULL MODEL %%
% MODEL FOR NMC LIB
if ChemType == 1

    thetaT   = coeff_T_nmc * ((temp_act - temp_ref_nmc) / temp_act);
    thetaDOD = coeff_DOD_nmc * DOD;
    thetaC   = coeff_Cch_nmc * CCrate + coeff_Cdch_nmc * DisCCrate;
    socShape = 1 + coeff_mSOC_nmc * mSOC * (1 - (mSOC / (2 * mSOC_ref_nmc)));

    SOH = 100 - beta_nmc * exp(thetaT + thetaDOD + thetaC) ...
        * socShape * FEC^(alpha_opt_nmc);

% MODEL FOR LFP LIB
elseif ChemType == 2

    thetaT   = coeff_T_lfp * ((temp_act - temp_ref_lfp) / temp_act);
    thetaDOD = coeff_DOD_lfp * DOD;
    thetaC   = coeff_Cch_lfp * CCrate + coeff_Cdch_lfp * DisCCrate;
    socShape = 1 + coeff_mSOC_lfp * mSOC * (1 - (mSOC / (2 * mSOC_ref_lfp)));

    SOH = 100 - beta_lfp * exp(thetaT + thetaDOD + thetaC) ...
        * socShape * FEC^(alpha_opt_lfp);
end
end
% ----------------------------------------------------------
